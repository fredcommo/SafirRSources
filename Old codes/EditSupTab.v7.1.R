EditSupTab.v7.1 <- function(stab = seg.table, CGH = Scgh, use.medians = TRUE, Select = c("Gain", "Loss", "Both"), Thresh.Gain = 1, Thresh.Loss = -1, CHR = 1:23, Restrict = FALSE, Root = "E"){
...
require(R2HTML)
	...
	linkModel <- list(Entrez = "http://www.ncbi.nlm.nih.gov/sites/entrez?db=gene&cmd=Retrieve&dopt=Graphics&list_uids=",
				...
				# http://ctdbase.org/detail.go?type=gene&acc=2067 CTD base
...
html_css <- function(filename, css_liste)
{
	...
	write.table("tr.firstline{background-color:#FFBD9D;}", file = filename, quote=F, append=T,col.names=F, row.names=F)			# fond ent�tes de sous-tables
	...
	write.table("a:visited{text-decoration:none;color:#8A008A;}", file = filename, quote=F, append=T,col.names=F, row.names=F)		# couleur du lien apr�s activation
	...
	write.table("h2{background-color:#FFA366;text-align:center;}", file = filename, quote=F, append=T,col.names=F, row.names=F)		# fond ent�te principal
...
# CTD.db <- read.csv("CTD_chem_gene_ixns.tsv", header = T, sep = "\t")
