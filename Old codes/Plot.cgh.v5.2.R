...
Plot.cgh.v5.2 <- function(Scgh, greq, hg19.info, Kmed = 3, cutGainLoss = 10, cutAmpDel = 85,
			probes.col = "grey90", gain.col = "dodgerblue3", loss.col = "red3", flat.col = "grey55",
			Y.expand = 0.5, Tag = TRUE, Dmin = 1.35e8, cex.text = 0.75, Tag.expand = 0.5, Equilibrate = TRUE, use.medians = T){
...
define.xpos <- function(x, xmin = 0, xmax = 3e9, dmin = Dmin, cex.text = cex.text){
	...
	if(dmin > (xmax - xmin)/(n-1)){
		...
		cat("Valeur minimale d'intervalles impossible à respecter\n")
...
data <- Scgh$cgh
	...
	# Graphique principal
		...
		plot(Rmed1~pos, pch = 19, cex = 0.25, col = pCol, ylim = range(-max.val, max.val), xlab = "Genome positioning", ylab = "Log2(ratio)")				# Valeurs des sondes
		points(pos[-which(chr == 24)], Rmed2[-which(chr == 24)], pch = ".", col = "black", cex = 1)												# visualisation de la segmentation
		...
		# Définition du titre
		Prop <- Scgh$Prop
		Prop <- round(length(which(abs(lr) > cutoff))/length(lr)*100, 2)
		Samp <- paste(	"Safir.Id: ", Scgh$Safir.Id, "  /  Sample.Id: ", Scgh$BarCode, "\n", "Scan.date: ", Scgh$ScanDate,
				" / Analysis: ", Scgh$analysisDate, " / Genome inst.(%): ", Prop, " (Cutoff = ", cutGainLoss, ")", sep = "")								# Définition du titre
