NormCGH.v5.test <- function(cgh, is.Agilent = TRUE, Cy = TRUE, GC = TRUE, Centr = TRUE, Fract = 1, span = 0.3, cut = c(0.05, 0.95), G = 3:7, peakThresh = 0.85, MergePeaks = T, MergeVal = 0.075, method = "Left", Plot = TRUE, Expand = 1.75, Save = TRUE, Root = "E"){
...
if(Centr){
	...
	#
		...
		dlr <- density(run.lr)
		max.dlr <- max(dlr$y)
		plot(dlr$x, dlr$y, type = "h", col = "grey75", xlab = "Log2R", ylab = "Density", xlim = range(-1.25, 1.25), ylim = range(0, max(dlr$y*1.25)))
