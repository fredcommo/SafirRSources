EditSupTab.v7 <- function(stab = seg.table, CGH = Scgh, Tech = "", use.medians = TRUE, Select = c("Gain", "Loss", "Both"), Thresh.Gain = 1, Thresh.Loss = -1, CHR = 1:23, Restrict = FALSE, Root = "E"){
...
require(R2HTML)
	...
	linkModel <- list(entrez = "http://www.ncbi.nlm.nih.gov/sites/entrez?db=gene&cmd=Retrieve&dopt=Graphics&list_uids=",
				...
				keggD = "http://www.genome.jp/kegg-bin/get_htext?htext=br08303_target.keg&query=",
				ct = "http://clinicaltrials.gov/ct2/results?term=")
...
## insertion d'une image dans un fichier html filename d�ja initialis�.
...
FileName <- paste(CGH$Safir.Id, CGH$BarCode, CGH$analysisDate, Tech, Explore, Select, "SupplTable", sep = "_")
	...
	if(length(Symbols)>0){
		cat("Symbols\n", Symbols, "\n")
			...
			if(length(Symbols)>0)
			{
				...
				## Création des adresses completes
				linkListe <- linkModel
				linkListe$entrez <- http_link(linkListe$entrez, tmp.request$GeneId)
				linkListe$kegg <- http_link(linkListe$Symb, tmp.request$Symb)
				linkListe$pgkb <- http_link(linkListe$pgkb,tmp.request$PgKB)
				linkListe$census <- http_link(linkListe$census,tmp.request$Cancer.Census.List)
				linkListe$keggD <- http_link(linkListe$keggD, tmp.request$KeggToDrugs)
				linkListe$ct <- http_link(linkListe$ctd, tmp.request$ClinicalTrials)

				## Création des liens html
				# linkListe <- sapply(linkListe, function(x){paste("<a href=", x, "target=_blank >", tmp.request$Symb, "</a>", sep="")})
				linkListe <- lapply(linkListe, function(x){paste("<a href=", x, "target=_blank >", GeneId, "</a>", sep="")})
				
				linkListe <- as.data.frame(linkListe)

				tab_name <- c(link_name,colnames(tmp.request[,c(2:3, 6:8, 12:13)]))
				
				# bug de formatage si il n'y a qu'un gène sur le fragment...
				if(ncol(linkListe)>1)
				{
					linkListe<-cbind(linkListe, tmp.request[,c(2:3, 6:8, 12:13)])
				}else{
					linkListe<-cbind(t(linkListe), tmp.request[,c(2:3, 6:8, 12:13)])
				}
				
				html_tab(filename=FileName, data=linkListe, caption=paste("<b>Segment on Chr", chr, ": ", start, "-", end, ", Log2R = ", seg.value, ", nb genes = ", length(Symbols),"</b>", sep = ""))
			}
		}
	}
	close(PB)
