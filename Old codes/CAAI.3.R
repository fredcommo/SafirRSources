...
CAAI.3 <- function(SegTable, alpha = 1e7, thetaP = 5e-3, thetaQ = 1.2){
	Score <- c()
	for(i in 1:24){
		pArm = paste(i, "p", sep = "")
		qArm = paste(i, "q", sep = "")
		...
		if(npSeg>1){
			...
			if(!is.null(tmpScore)){
				if(nrow(tmpScore)>1) pScore <- sum(apply(tmpScore[,1:3], 1, prod))
				else pScore <- prod(tmpScore[1:3])
				}
			}
		...
		if(nqSeg>1){
			...
			if(!is.null(tmpScore)){
				if(nrow(tmpScore)>1) qScore <- sum(apply(tmpScore[,1:3], 1, prod))
				else qScore <- prod(tmpScore[1:3])
				}
			}
