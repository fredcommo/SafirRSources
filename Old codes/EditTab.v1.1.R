...
EditTab.v1.1 <- function(myTab = greq, Root = "E"){
...
################################################################
	...
	linkList2 <- cbind.data.frame(myTab[,3:4], linkList2, myTab[,c(6:7, 10:11, 15:17, 20:22)])		# Information first(see above for #col concordance), then links.
	linkList2$Status <- paste("<span id=", linkList2$Status, "", ">", linkList2$Status, "</span>", sep = "")
	colnames(linkList2)[c(4, 6:12)] <- c("Chr#", "GenomicStart", "GenomicEnd", "Segm.Start", "Segm.End", "Segm.Length", "Log2R", "FoldChg")
...
html_css <- function(filename, css_liste)
{
	...
	write.table("#Norm{color:grey;}",file=filename, quote=F, append=T,col.names=F, row.names=F)
	write.table("#Gain{color:dodgerblue;}",file=filename, quote=F, append=T,col.names=F, row.names=F)
	write.table("#Ampli{color:darkblue;}",file=filename, quote=F, append=T,col.names=F, row.names=F)	
