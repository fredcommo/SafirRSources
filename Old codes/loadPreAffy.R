...
loadPreAffy <- function(FileName, Safir.Id, hg19.info){
...
a.info <- try(read.csv(paste(FileName, sep = ""), header = F, fill = T, skip = 0, nrows = 500, sep = "="), silent = T)

	{
	if(class(a.info)!="try-error"){
		datadate <- which(a.info[,1] == "#cn-annotation-create_date")		# "#state-time-start"
