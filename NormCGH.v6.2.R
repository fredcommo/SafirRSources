NormCGH.v6.2 <- function(cgh, is.Agilent = TRUE, Cy = TRUE, GC = TRUE, Centr = TRUE, Fract = 1, span = 0.3, cut = c(-0.5, 0.5), G = 3:7, peakThresh = 0.75, MergePeaks = T, MergeVal = 0.075, method = "Left", Plot = TRUE, Expand = 1.75, useX11 = FALSE, Save = TRUE, Root = "E"){
...
if(Centr){
	...
	#
		...
		max.dlr <- max(dlr$y)
		maxy <- max(dlr$y)*1.25
		plot(dlr$x, dlr$y, type = "n", xlab = "Log2R", ylab = "Density", xlim = range(q1*Expand, q2*Expand), ylim = range(0, maxy))
