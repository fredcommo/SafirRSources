...
GeneRequest_NM(enCours) <- function(genelist, hg19.info, DB = "unigene", verbose = TRUE){
...
gsummary <- function (id, database){
	...
	sapply(c("//Item name"), xpathApply, doc = doc, fun = xmlValue)
	}
...
# End fonctions requêtes
	...
	for(i in 1:Total){
		...
		gname <- genelist[i]
		ids <- unlist(gsearch(paste(gname, "mRNA homo sapiens"), database = DB))					#homo sapiens
		...
		if(!is.null(ids)){ 
			...
			j = 1
			Id <- ids[j]
			gsum <- unlist(gsummary(paste(Id, "mRNA homo sapiens"), database = DB))					#homo sapiens
    			TITLE <- gsum[1]
			CHROMOSOME <- gsum[2]
			ORGANISM <-  <- gsum[3]
    			GENE <- gsum[4]
		...
		gene.annot <- rbind(gene.annot, c(GENE, TITLE, CHROMOSOME, ORGANISM, CLUSTERID , TAXID, SEQ_COUNT, EST_COUNT, GENEID, RECORDTYPE))
		}	
