...
EditSupTab.v7.6 <- function(stab = seg.table, CGH = Scgh, ReqFunc = Request, use.medians = TRUE, 
...
################################################################
	...
	TitleName <- paste("Supplementary tables: ", CGH$Safir.Id, " / ", CGH$BarCode, "<br>",
						CGH$Tech, " / ", CGH$analysisDate, " / ", paste("Chr", CHR[1], "to", CHR[length(CHR)]), ": ", paste("segments", selecType))
	...
	for(i in whichseg){
		...
		if(length(Symbols)>0){
			...
			if(length(Symbols)>0){
				...
				if(sangercensus) tmp.request <- cbind.data.frame(tmp.request, Cancer.Census.List = ifelse(!is.na(is.census), as.character(is.census), "-"))
				if(pharmgkb) tmp.request <- cbind.data.frame(tmp.request, PgKB = pgkb.Ids)
				if(keggtodrug) tmp.request <- cbind.data.frame(tmp.request, KeggToDrugs = ifelse(!is.na(Kegg.GI), as.character(Kegg.GI), "-"))
				if(ctdbase) tmp.request <- cbind.data.frame(tmp.request, CTDbase = ifelse(!is.na(is.ctd), as.character(tmp.request$GeneId), "-"))
				if(clintrials) tmp.request <- cbind.data.frame(tmp.request, ClinicalTrials = ifelse(!is.na(CT.Symb), as.character(CT.Symb), "-"))
				...
				if(sangercensus) linkListe$SangerCensus <- http_link(linkListe$SangerCensus, tmp.request$Cancer.Census.List)
				if(pharmgkb) linkListe$PharmgKB <- http_link(linkListe$PharmgKB, tmp.request$PgKB)
				if(keggtodrug) linkListe$KeggToDrugs <- http_link(linkListe$KeggToDrugs, tmp.request$KeggToDrugs)
				if(ctdbase) linkListe$CTDbase <- http_link(linkListe$CTDbase, tmp.request$CTDbase)
				if(clintrials) linkListe$ClinicalTrials <- http_link(linkListe$ClinicalTrials, tmp.request$ClinicalTrials)
